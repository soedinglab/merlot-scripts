...
svg("/save/path/prosstt_dists.svg", width = 7, height = 5)
boxplot(prosstt_dists,
        main="On-tree normalized distance from origin",
        xlab="Time from origin (pseudotime units)",
        ylab="Normalized on-tree distance (average branch lengths)",
        xaxt="n", ylim=c(0.6, 2.0), xlim=c(0.5, 9.5))
box(which ="plot", lwd=2)
keep <- prosstt_where > -Inf
text(x=1:sum(keep), y=prosstt_where[keep] + 0.07, labels=prosstt_howmany[keep])
axis(1, at=1:9, labels = (1:9)*50)
abline(h = 1, lty=2)
dev.off()

svg("/save/path/splatter_dists.svg", width = 7, height = 5)
boxplot(splatter_dists,
        main="On-tree normalized distance from origin",
        xlab="Time from origin (pseudotime units)",
        ylab="Normalized on-tree distance (average branch lengths)",
        xaxt="n", ylim=c(0.6, 2.), xlim=c(0.5, 9.5))
keep <- splatter_where > -Inf
text(x=1:sum(keep), y=splatter_where[keep] + 0.07, labels=splatter_howmany[keep])
axis(1, at=1:9, labels = (1:9)*50)
abline(h = 1, lty=2)
box(which ="plot", lwd=2)
dev.off()